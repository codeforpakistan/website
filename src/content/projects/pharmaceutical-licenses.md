---
title: "Digitisation of Pharmaceutical Licenses in KP"
slug: "pharmaceutical-licenses"
image: "/media/projects/project-pharmaceutical-licenses.jpg"
department: "Department of Health and Directorate General Drugs"
year: ""
sortOrder: 6
---

The Department of Health and Directorate General Drugs contacted the KP Government Innovation Fellowship team with a request to digitise and automate the process of issuing licenses to drug stores and pharmacies in KP. Previously, there was no centralised online system for drug store owners to apply online, which could save their time and money and expedite the process on the department's end. Similarly, the Health Department also encountered issues in tracking duplicate and fake licenses, resulting in illegal pharmacies operating throughout the province.

After analysing and understanding the requirements and needs of the Health Department, the Fellowship team created a fully centralised online platform to digitise the process for license application, renewal, and verification for pharmacies in KP Province.

With this new online platform:

- Users can easily apply for new licenses or renew existing licenses
- Already issued licenses can be verified for authenticity
- The department can check for duplicate and fake licenses
- Users can track the progress of obtaining a license

**Impact**

Since its launch in 2021, this digital solution has transformed the entire process of pharmaceutical license application, renewal, and verification.

- So far over 20,000 licenses have been issue using the online system
- It is enabling the department to streamline their record-keeping and improve oversight for detecting discrepancies and other shortcomings in the license issuing processes.
- It has improved the speed of issuing licenses.
- It is enabling performance tracking of inspectors
